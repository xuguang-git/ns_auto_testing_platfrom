<template>
  <div class="database-page">
    <PageHeader title="数据库管理" description="按环境维护只读数据库连接，配置 SELECT 查询数据源供接口测试前后置使用。" />
    ...
    <el-tabs v-model="activeTab" class="database-tabs">
      <el-tab-pane label="连接配置" name="connections">
        <section class="database-toolbar">
          ...
          <el-input v-model="connectionKeyword" clearable placeholder="搜索连接名称" style="width: 260px" />
        ...
        <section class="database-card">
          <el-table :data="filteredConnections" v-loading="loading" stripe height="100%">
            ...
            <el-table-column label="类型" width="100">
              <template #default="{ row }">{{ databaseTypeText(row.db_type) }}</template>
            </el-table-column>
            <el-table-column label="状态" width="110">
              <template #default="{ row }">
                <span class="badge" :class="checkBadge(row.last_check_status)">{{ checkStatusText(row.last_check_status) }}</span>
              </template>
            </el-table-column>
      ...
      <el-tab-pane label="查询数据源" name="sources">
        <section class="database-toolbar">
          <el-select v-model="sourceConnectionFilter" clearable placeholder="数据库连接" style="width: 210px">
        ...
        <section class="database-card">
          <el-table :data="filteredSources" v-loading="loading" stripe height="100%">
            ...
            <el-table-column label="SQL" min-width="320" show-overflow-tooltip>
              <template #default="{ row }"><span class="inline-code">{{ row.sql }}</span></template>
            </el-table-column>
            <el-table-column label="提取变量" min-width="180" show-overflow-tooltip>
              <template #default="{ row }">{{ extractorText(row.extractors) }}</template>
            </el-table-column>
    ...
    <el-dialog v-model="sourceDialog" :title="sourceForm.id ? '编辑查询数据源' : '新增查询数据源'" width="760px">
      <el-form :model="sourceForm" label-width="110px">
        ...
        <el-form-item label="数据库连接" required>
          <el-select v-model="sourceForm.database_connection" filterable style="width: 100%" @change="syncSourceEnvironment">
            <el-option v-for="connection in activeConnections" :key="connection.id" :label="`${connection.name} / ${connection.environment_name}`" :value="connection.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="SQL" required>
          <el-input v-model="sourceForm.sql" type="textarea" :rows="8" placeholder="SELECT id, order_no FROM orders WHERE user_id = '{{user_id}}'" />
        </el-form-item>
        ...
        <el-form-item label="提取规则">
          <div class="extractor-editor">
            <div class="extractor-row extractor-head">
              <span>变量名</span><span>列名</span><span>模式</span><span>行号</span><span></span>
            </div>
            <div v-for="(extractor, index) in sourceForm.extractors" :key="extractor.uid" class="extractor-row">
              <el-input v-model="extractor.name" placeholder="order_id" />
              <el-input v-model="extractor.column" placeholder="id" />
              <el-select v-model="extractor.mode">
                <el-option label="首行/指定行" value="first" />
                <el-option label="整列数组" value="column" />
              </el-select>
              <el-input-number v-model="extractor.row" :min="0" :max="999" controls-position="right" />
    ...
    <el-dialog v-model="runDialog" title="查询数据源试运行" width="760px">
      <div v-if="runResult" class="source-run-result">
        <el-descriptions :column="3" border>
          <el-descriptions-item label="行数">{{ runResult.row_count }}</el-descriptions-item>
          <el-descriptions-item label="提取变量" :span="2">{{ Object.keys(runResult.variables || {}).length }}</el-descriptions-item>
        </el-descriptions>
        <h4>Variables</h4>
        <pre>{{ JSON.stringify(runResult.variables || {}, null, 2) }}</pre>
        <h4>Rows Preview</h4>
...
const sourceForm = reactive({
  ...
  database_connection: undefined as number | undefined,
  sql: "",
  extractors: [] as ExtractorForm[],
  description: "",
  is_active: true,
});
...
const filteredConnections = computed(() => connections.value.filter((item) => {
  ...
  return (!connectionKeyword.value || text.includes(connectionKeyword.value.toLowerCase())) &&
    (!connectionEnvFilter.value || item.environment === connectionEnvFilter.value);
}));
const filteredSources = computed(() => sources.value.filter((item) => {
  const text = `${item.name} ${item.sql}`.toLowerCase();
  return (!sourceKeyword.value || text.includes(sourceKeyword.value.toLowerCase())) &&
    (!sourceConnectionFilter.value || item.database_connection === sourceConnectionFilter.value);
}));
...
const load = async () => {
  loading.value = true;
  try {
    const [envResp, connectionResp, sourceResp] = await Promise.all([
      platformApi.environments(),
      platformApi.databaseConnections(),
...
const syncSourceEnvironment = () => {
  const connection = connections.value.find((item) => item.id === sourceForm.database_connection);
  sourceForm.environment = connection?.environment;
};
...
const openSourceDialog = (row?: any) => {
  Object.assign(sourceForm, {
    ...
    database_connection: row?.database_connection || activeConnections.value[0]?.id,
    sql: row?.sql || "",
    extractors: (row?.extractors?.length ? row.extractors : [{ name: "", column: "", mode: "first", row: 0 }]).map((item: any) => ({
      uid: Date.now() + Math.floor(Math.random() * 10000),
      name: item.name || "",
      column: item.column || item.path || "",
      mode: item.mode || "first",
...
const buildExtractors = () => sourceForm.extractors
  .filter((item) => item.name.trim() && item.column.trim())
  .map((item) => ({ name: item.name.trim(), column: item.column.trim(), mode: item.mode, row: item.row || 0 }));

const saveSource = async () => {
  if (!sourceForm.name.trim() || !sourceForm.database_connection || !sourceForm.sql.trim()) {
    ElMessage.warning("请填写数据源名称、数据库连接和 SQL");
    return;
  }
  if (sqlWarning.value) {
    ElMessage.warning(sqlWarning.value);
    return;
  }
  saving.value = true;
  try {
    ...
    const payload = {
      ...
      database_connection: sourceForm.database_connection,
...
const removeSource = async (row: any) => {
  await ElMessageBox.confirm(`确认删除查询数据源「${row.name}」？`, "删除确认", { type: "warning" });
  await platformApi.deleteTestDataSource(row.id);
  ElMessage.success("查询数据源已删除");
  await load();
};
